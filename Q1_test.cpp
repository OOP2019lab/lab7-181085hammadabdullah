...
#include "Q1_header.h"
//
bankAccount **readfile(string file,int &size1);
void printAccounts(bankAccount **arr);
bool makeTransaction(bankAccount **arr,int id,int amount,int size);
	...
	//
{
	...
	//
	bankAccount *arr[2];
	arr[0]= new bankAccount(30,2000,100000); //daefault Balance limit
	arr[0]->deposit(3000);
	arr[0]->deposit(5000);
	arr[0]->deposit(60000);
	...
	//	//*****Write your code here
	//	*/
	bankAccount **arr1=new bankAccount*[2];
	arr1[0]=new bankAccount(50,2000,1000000); //deafult balance limit
	arr1[0]->withdraw(5000);
	arr1[0]->deposit(5000);
	arr1[1]=new bankAccount(60,6000,400000);
	arr1[0]->withdraw(1000);
	arr1[0]->print();
	arr1[1]->print();
	//
	//
	...
	int size=0;
	//	*/
	bankAccount **arr2=readfile("file.txt",size);
	//	/********************************TASK 7*************************************
	...
	//	*/
	//
	cout<<makeTransaction(arr2,5,-6000,size);
	//	/********************************TASK 9*************************************
...
bankAccount **readfile(string file,int &size1){
	...
	for(int i=0;i<size;i++){
		...
		fin>>limit;
		fin>>nooftran;
		arr[i]=new bankAccount(id,bal,limit);
		float *tran=new float[nooftran];
		for(int i=0;i<nooftran;i++){
		fin>>tran[i];
		}
		bankAccount(id,bal,tran,nooftran,limit);
	  getline(fin,buff);//To skip line
	}
	return arr;
	fin.close();
}
...
bool makeTransaction(bankAccount **arr,int id,int amount,int size){
	for(int i=0;i<size;i++){
		if(arr[i]->getID==id){
			if(amount>0 && arr[i]->deposit(amount)==true){
			cout<<"Transcition Suceessfull"<<endl;
			return true;
			}
			else if (amount<0 && arr[i]->withdraw(amount)==true){
			cout<<"Transition successful"<<endl;
			return true;
			}
			else{
				cout<<"Transiciton Fails!!!!"<<endl;
			return false;
